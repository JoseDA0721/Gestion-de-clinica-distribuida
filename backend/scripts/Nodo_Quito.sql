-- Script Nodo Quito (26.44.192.25)

-- Secuencias para IDs serializables con prefijo
CREATE SEQUENCE seq_inventario_quito START 1;
CREATE SEQUENCE seq_cita_quito START 1;
CREATE SEQUENCE seq_receta_quito START 1;
CREATE SEQUENCE seq_historial_quito START 1;
CREATE SEQUENCE seq_notas_quito START 1;

-- Historial clínico (fragmentación vertical derivada)
CREATE TABLE HistorialClinico_Base_Quito (
    HistorialID VARCHAR PRIMARY KEY DEFAULT ('HCQ' || LPAD(nextval('seq_historial_quito')::TEXT, 3, '0')),
    CitaID VARCHAR,
...
CREATE TABLE HistorialClinico_Detalle_Quito  (
...
CREATE TABLE Pacientes_Quito  (
    ...
    FechaCreacion TIMESTAMP NOT NULL,
    CHECK (SedeRegistroID = 1)
);

-- Doctores
CREATE TABLE Doctores_Quito  (
    ...
    NodoOrigen VARCHAR(50),
    FechaCreacion TIMESTAMP NOT NULL,
    CHECK (SedePrincipalID = 1)
);

-- Citas
CREATE TABLE Citas_Quito  (
    CitaID VARCHAR PRIMARY KEY DEFAULT ('CQ' || LPAD(nextval('seq_cita_quito')::TEXT, 3, '0')),
    CedulaPaciente VARCHAR,
...
-- Recetas
CREATE TABLE Recetas_Quito  (
    RecetaID VARCHAR PRIMARY KEY DEFAULT ('RQ' || LPAD(nextval('seq_receta_quito')::TEXT, 3, '0')),
    CitaID VARCHAR,
    MedicamentoID INT,
    Dosis VARCHAR,
    Frecuencia VARCHAR,
...
CREATE TABLE InventarioMedicamentos_Quito  (
    InventarioID VARCHAR PRIMARY KEY DEFAULT ('IQ' || LPAD(nextval('seq_inventario_quito')::TEXT, 3, '0')),
    SedeID INT,
    MedicamentoID INT,
...
SELECT pglogical.create_node(
    node_name := 'nodo_quito_test',
    dsn := 'host=26.44.192.25 port=5432 dbname=clinica_quito user=postgres password=P@ssw0rd'
);
...
SELECT pglogical.create_subscription(
    ...
    provider_dsn := 'host=26.26.58.209 port=5432 dbname=clinica_guayaquil user=postgres password=P@ssw0rd',
